...
if ($hook) { $ch = curl_init($hook); curl_setopt_array($ch, [CURLOPT_POST=>1, CURLOPT_POSTFIELDS=>json_encode($d, JSON_UNESCAPED_UNICODE), CURLOPT_HTTPHEADER=>['Content-Type: application/json'], CURLOPT_TIMEOUT=>5, CURLOPT_RETURNTRANSFER=>1]); @curl_exec($ch); curl_close($ch); }
// 메일 알림(사진 제외)
$sum = "새 견적 문의 [$id]\n유형: {$d['type']}\n평수: {$d['size']}\n높이: {$d['height']}\n오염: {$d['hazard']}\n희망일: {$d['when']}\n연락처: {$d['contact']}\n예상: {$d['estimate']}\n사진: " . count($d['photos'] ?? []) . "장\n";
